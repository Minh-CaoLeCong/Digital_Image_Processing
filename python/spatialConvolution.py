...
# USAGE
# python spatialConvolution.py --input imagePath
# example:  python spatialConvolution.py --input E:\\CVS\\Project\\Digital_Image_Processing\\python\\image\\spatialConvolution.tif
#           python spatialConvolution.py --input E:/CVS/Project/Digital_Image_Processing/python/image/spatialConvolution.tif
...
def spatialConvolution(imageInput, kernel):
    """
    """
...
# display spatial convolution processing image
cv2.imshow('spatialConvolution', imageSpatialConvolution)

print("[INFOR]: Press 's' key to save result")
print("[INFOR]: Press 'q' key to quit")
key = cv2.waitKey(0) & 0xFF
if key == ord("s"):
    resultPath = "./output/" + "spatialConvolution_" + imageFileName
    cv2.imwrite(resultPath, imageSpatialConvolution)